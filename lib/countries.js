...
async function main(participants, startDate, stopDate, format, homeAway = 'both') {
  ...
  if (format === 'jsonld') {
    ...
    return countriesToJSONLD(rankingArray, `https://dancehallbattle.org/ranking/country/${participants.join('-')}/${homeAway}/` + formatDate(new Date(), 'yyyy-MM-dd'), homeAway);
  } else if (format === 'csv') {
    return generateCSV(rankingArray);
  } else {
