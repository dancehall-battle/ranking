...
class DancerRanker extends Ranker {
  ...
  _toJSONLD(rankingArray) {
    const types = ['dhb:DancerRanking'];

    if (this.options.participants.length === 1) {
      if (this.options.participants[0] ===  '1') {
        types.push('dhb:1vs1Ranking');
      } else {
        types.push('dhb:2vs2Ranking');
      }
    }

    rankingArray.forEach(rank => {
      rank.dancer = {'@id': rank.dancer}
    });
