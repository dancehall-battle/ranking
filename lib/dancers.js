...
async function main(participants, startDate, stopDate, format = 'csv', homeAway = 'both') {
  ...
  if (format === 'jsonld') {
    ...
    console.log(JSON.stringify(dancersToJSONLD(rankingArray, `https://dancehallbattle.org/ranking/dancer/${participants.join('-')}/${homeAway}/` + formatDate(new Date(), 'yyyy-MM-dd'), homeAway)));
  } else if (format === 'csv') {
