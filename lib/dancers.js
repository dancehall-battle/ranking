...
async function main(participants, startDate, stopDate, format) {
  // Execute the query
  ...
  if (format && format === 'jsonld') {
    ...
    console.log(JSON.stringify(toJSONLD(rankingArray, `https://dancehallbattle.org/ranking/dancer/${participants.join('-')}/` + formatDate(new Date(), 'yyyy-MM-dd'))));
  } else {
