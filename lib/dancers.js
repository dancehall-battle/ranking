const {setRankings, dancersToJSONLD, getDancers, toJSONLD}  = require('./utils');
...
class DancerRanker extends Ranker {
  ...
  _getRankingArray(dancers) {
    const dancerToPoints = this._getPoints(dancers);
    const rankingArray = [];
  ...
  _toJSONLD(rankingArray, homeAway) {
    return toJSONLD(rankingArray, `https://dancehallbattle.org/ranking/dancer/${this.options.participants.join('-')}/${homeAway}/` + formatDate(new Date(), 'yyyy-MM-dd'), homeAway, {
