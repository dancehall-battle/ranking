...
class Ranker {
  ...
  async getRanking(options) {
    ...
    const dancers = await getDancers(query, options.startDate, options.endDate, options.participants, options.homeAway);
    ...
    if (options.format === 'jsonld') {
      ...
      return this._toJSONLD(rankingArray, options.homeAway);
