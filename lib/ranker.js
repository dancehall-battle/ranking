...
class Ranker {
  ...
  async getRanking(options) {
    ...
    if (options.format === 'jsonld' || options.format === 'ntriples') {
      //console.dir(toJSONLD(rankingArray), {depth: null});
      let doc = this._toJSONLD(rankingArray);

      if (options.format === 'ntriples') {
        return await toRDF(doc, {format: 'application/n-quads'});
      } else {
        return doc;
      }
  ...
  async _getDancers(query, startDate, stopDate, participants, homeAway, tpfServer, removeFemaleBattles = false, scale = false) {
